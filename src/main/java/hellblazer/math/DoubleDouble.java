...
public strictfp class DoubleDouble
         ...
         * ******************************************************** *
         * **********                                    ********** *
         * **********  Yurelle: Added for Compatibility  ********** *
         * **********                                    ********** *
         * ******************************************************** *
         * ******************************************************** *
         */
        extends SupportedNumeric<DoubleDouble>
        /* END ADDED*/

        implements Serializable, Comparable<DoubleDouble>, Cloneable {
     ...
     */
    @Override
    public DoubleDouble getFor(int intNum) {
    ...
    /**
     * The value nearest to the constant Zero; Yurelle added this. This needs to be defined LAST.
     */
    ...
    public static DoubleDouble parse(String str) throws NumberFormatException {
        //TODO a hack work around. The string parsing is not working for some reason (always returns zero). As a quick fix
        //that does not require an app-wide switch of constructor calls, we'll just bypass it here.
        if (true) {
            return new DoubleDouble(Double.parseDouble(str));
        } else {
            int i = 0;
    ...
    public static void main(String[] args) {
        ...
        x = x.add(new DoubleDouble("1234"));
        System.out.println(x.intValue());
        x = x.multiply(new DoubleDouble("10"));
        System.out.println(x.intValue());
        x = x.divide(new DoubleDouble("5"));
        System.out.println(x.intValue());

        x = new DoubleDouble("81");
        System.out.println(x.intValue());
        x = x.sqrt();
        System.out.println(x.intValue());
    }
