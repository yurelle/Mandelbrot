...
public class AppContext {
    ...
    ArrayList<PrecisionContext<? extends SupportedNumeric<?>>> precisionContexts = new ArrayList<>() {{
        add(new PrecisionContext<>(new MandelFloat()));
        add(new PrecisionContext<>(new MandelDouble()));
        add(new PrecisionContext<>(new MandelDD()));
        add(new PrecisionContext<>(new MandelBigD()));
    }};

    public PrecisionContext<? extends SupportedNumeric<?>> getPrecisionContext() {
        return precisionContexts.get(CURRENT_PRECISION);
    }

    public MathALU<? extends SupportedNumeric<?>> getALU() {
        return precisionContexts.get(CURRENT_PRECISION).ALU;
    }
    ...
    String[] menuText = new String[] {
        ...
        "[UP] Increase Precision",
        "[DOWN] Decrease Precision",
    ...
    Viewport<? extends SupportedNumeric<?>> ORIG_VIEWPORT;
    Viewport<? extends SupportedNumeric<?>> CURRENT_VIEWPORT;

    Stack<Viewport<? extends SupportedNumeric<?>>> viewportHistory = new Stack<>();
...
//        pixelDensity(1);
        ...
        //Get Context Handles
        final PrecisionContext context = getPrecisionContext();
        final MathALU<? extends SupportedNumeric<?>> ALU = context.ALU;

        //Init Viewports
        ORIG_VIEWPORT = context.createViewport();
        ORIG_VIEWPORT.setMinX("-2.25");
        ORIG_VIEWPORT.setMaxX("0.75");
        ORIG_VIEWPORT.setMinY("-1.5");
        ORIG_VIEWPORT.setMaxY("1.5");
    ...
    Viewport<? extends SupportedNumeric<?>> pushCurrentViewport() {
        Viewport<? extends SupportedNumeric<?>> vp = CURRENT_VIEWPORT.clone();
...
//        workerThread = new Thread(MathALU::calcMandelFrame);//TODO
//        workerThread = new Thread(MathALU_BigD::calcMandelFrame);
//        workerThread = new Thread(MathALU_DD::calcMandelFrame);
        workerThread = new Thread(() -> getPrecisionContext().ALU.calcMandelFrame());
        workerThread.start();
    }

    public void setNewViewport(int originX, int originY, int boxSize) {
        // Save Previous Viewport
        final Viewport previousViewport = pushCurrentViewport();

        //Calculate Bounds
        CURRENT_VIEWPORT.mapMinX(originX,           0, width,    previousViewport);
        CURRENT_VIEWPORT.mapMinY(originY,           0, height,   previousViewport);
        CURRENT_VIEWPORT.mapMaxX(originX + boxSize, 0, width,    previousViewport);
        CURRENT_VIEWPORT.mapMaxY(originY + boxSize, 0, height,   previousViewport);
